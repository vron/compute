...
int32_t findLSB(int32_t value) { return __builtin_ffs(value) - 1; }

int32_t findLSB(uint32_t value) { return __builtin_ffs(value) - 1; }
...
uint32_t clamp(uint32_t x, uint32_t mi, uint32_t ma) {
...
int32_t bitCount(int32_t value) { return __builtin_popcount(value); }

int32_t bitCount(uint32_t value) { return __builtin_popcount(value); }
...
vec2 unpackSnorm2x16(uint32_t p) {
  float v1 = (float)(p & 0xFFFF);
  float v2 = (float)(p & 0xFFFF0000);
  return make_vec2(clamp(v1 / 32727.0f, -1.0, 1.0),
                   clamp(v2 / 32727.0f, -1.0, 1.0));
}
...
vec4 unpackSnorm4x8(uint32_t p) {
  ...
  float v3 = (float)(p & 0xFF0000);
  float v4 = (float)(p & 0xFF000000);
  return make_vec4(clamp(v1 / 127.0f, -1.0, 1.0), clamp(v2 / 127.0f, -1.0, 1.0),
...
//    return roundf(x);
//}
vec2 round(vec2 x) { return make_vec2(roundf(x[0]), roundf(x[1])); }
...
vec2 sign(vec2 x) { return make_vec2(sign(x[0]), sign(x[1])); }
vec3 sign(vec3 x) { return make_vec3(sign(x[0]), sign(x[1]), sign(x[2])); }
vec4 sign(vec4 x) {
  return make_vec4(sign(x[0]), sign(x[1]), sign(x[2]), sign(x[3]));
}
...
ivec2 sign(ivec2 x) { return make_ivec2(sign(x[0]), sign(x[1])); }
